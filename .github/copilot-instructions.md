...
import (
    ...
    "github.com/qualithm/operator-mcp/internal/config"
)
```
...
- Use `goimports -local github.com/qualithm/operator-mcp` to keep local packages last.
...
`./ci-sync.sh` from the dx repo. Branch rulesets in `dx/rulesets/` enforce a single required status
